...
import { consultarComprasSemLicitacao } from '@/services/comprasGovService';
...
const formSchema = z.object({
  dtAnoAviso: z.preprocess(val => Number(val), z.number({ required_error: 'Ano do aviso é obrigatório.' }).min(1990).max(currentYear + 5)),
  ...
  coModalidadeLicitacao: z.preprocess(val => val ? Number(val) : undefined, z.enum(['6', '7']).optional()), // 6 (Dispensa), 7 (Inexigibilidade)
});

type FormValues = z.infer<typeof formSchema>;

export default function ConsultarComprasSemLicitacaoPage() {
  const defaultValues: FormValues = {
    dtAnoAviso: currentYear,
    pagina: 1,
    tamanhoPagina: 10,
  };
  ...
  const renderForm = (form: ReturnType<typeof useForm<FormValues>>) => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <FormField
        ...
        render={({ field }) => (
          <FormItem>
            <FormLabel>Ano do Aviso*</FormLabel>
            <FormControl>
  ...
  return (
    <ApiConsultaForm
      ...
      fetchDataFunction={consultarComprasSemLicitacao}
      formTitle="Consultar Compras Sem Licitação"
      formDescription="Busque informações sobre Dispensas e Inexigibilidades."
    />
  );
}
